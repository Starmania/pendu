...
def init:
    ...
    while jveux_continuer:
        try:
            with codecs.open("data\\Base de donnees mots francais.txt",  "r", encoding='utf-8') as mots: # On choisit un mot au hasard dans la base de données
                mot_random = mots.readlines()[random.randint(0,22740)] 
        except FileNotFoundError: # On vérifie que le fichier en question est bien présent, si ce n'est pas le cas, on 
                                  # le redirige vers ce dernier par l'intermédiaire du lien ci-joint
        ...
        while mot_mystere != mot_random:
            ...
            compteur = 0
            lettres_trouvees = 0
            for i in mot_random: # On vérifie si le joueur a trouvé des lettres parmi le mot tiré au hasard
            if touche == i:
